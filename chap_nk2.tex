...
%celui-ci est capable de pointer à des coordonnées voulues, et qu'il est correctement focalisé.
...
\begin{table}[t]
    ...
    \begin{tabular}{l r r}
        ...
        %Fréquence centrale [GHz]            & 254.7 et 257.4 & 150.9 \\
        \midrule
        ...
        %Largeur à mi-hauteur (FWHM) du lobe de référence [arcsec] & $12.5$ & $18.5$ \\
        %Efficacité du lobe de référence [\%]    & $61 \pm 3$ & $72 \pm 3$ \\
...
%au processus de reconstruction des fréquences de résonance dans le plan focal; un détecteur peut donc être invalide pendant une campagne d'observation, mais valide la suivante.
%Les valeurs reportées en table \ref{tab:nk_specs} doivent donc être vues comme une indication de la performance de l'instrument pendant les campagnes de \textit{commissioning}, non comme une de ses caractéristiques constantes\footnote{C'est aussi une bonne nouvelle: puisque des détecteurs ne le sont pas définitivement et peuvent redevenir valides, on ne s'attend pas à ce que cette fraction se dégrade avec le temps.}.
...
%, et l'anneau de diffraction correspondant au second pic de la tache d'Airy (voir section \ref{sec:30m_opt}, \pageref{sec:30m_opt}).
...
%la dispersion de différentes mesures de la densité de flux en provenance de la galaxie HLS J0918+5142.
...
La sélection de l'échantillon a donc été basée sur les catalogues SZ existant au début du programme, à savoir les catalogues \textit{Planck} et ACT \cite{planck_collaboration_planck_2016-3, hasselfield_atacama_2013}.
...
\setlength\itemsep{5pt}
    ...
    \item Les profils d'autres propriétés thermodynamiques du milieu intra-amas obtenues par combinaison avec des données externes, notamment dans le domaine des rayons X; %-- voir discussion ci-après;
    ...
    %pour des combinaisons des données du grand programme SZ dans des analyses cosmologiques ajustant la relation d'échelle masse-observable simultanément avec la cosmologie -- voir section \ref{sec:scaling}.
\end{itemize}
...
Le profil universel d'\myciteauthor{arnaud_universal_2010}, obtenu par l'exploitation de l'échantillon REXCESS en X à $z < 0.2$, a été utilisé dans la construction de plusieurs catalogues d'amas en SZ, notamment par les collaborations ACT \cite{hilton_atacama_2021} et \textit{Planck} \cite{planck_collaboration_planck_2016-3}.
...
Quel impact aurait une variation du profil de pression moyen sur les analyses cosmologiques basées sur l'effet SZ? \\
    ...
    Cet impact a été quantifié dans le cas d'analyses du spectre de puissance de l'effet SZ par \myciteauthor{ruppin_impact_2019}, en analysant la carte de paramètre de Compton issue des données de \textit{Planck} \cite{planck_collaboration_planck_2016-4} avec différents profils de pression moyens.
...
En effet, les amas du grand programme SZ sont en majorité issu du catalogue \textit{Planck} \cite{planck_collaboration_planck_2016-3}, construit à partir d'observations dont la résolution est proche de la taille du champ de vue de NIKA2.
...
Un autre exemple d'utilisation est la possibilité d'avoir un outil permettant de connaître rapidement le nombre de \textit{scans} ayant été effectués sur chaque amas peut grandement faciliter la prise de décision sur la liste de sources choisies pour être observées au télescope.
...
D'autre part, la base de donnée doit pouvoir être utilisée à plusieurs étapes de la chaîne d'analyse du grand programme SZ:
\begin{itemize}[leftmargin=*]
