...
class Data:
    ...
    def plot_data(self, style="errb", color="k"):
        """
        Make (x, y) plot
        ...
        if style == "ellipse":
            ...
            ax.plot(self.x_obs, self.y_obs, "D", color=color, ms=2)
        elif style == "errb":
            ax.errorbar(
                self.x_obs, self.y_obs, xerr=self.x_err, yerr=self.y_err, mfc="w", **kw
            )
        else:
            ax.plot(self.x_obs, self.y_obs, "o", mfc="w", mec=color)
    ...
    def plot_alphabeta(
        ...
        if isinstance(alpha, float):
            ...
            ax.fill_between(x, pct[0], pct[1], alpha=0.3, **kwargs)
