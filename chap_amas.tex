...
\footnotetext{Nous discuterons de l'obtention de telles cartes au chapitre \ref{chap:decorr}.}
...
\footnotetext{La carte $\mathbf{N}$ n'est pas connue \textit{stricto sensu}, mais un estimateur peut être calculé grâce au spectre de puissance des cartes de demi-différences ; voir section \ref{sec:noise_pk}.}
...
L'étalonnage de la relation d'échelle est effectuée en étudiant le lien entre masse des amas et observables pour un échantillon de masses connues.
...
\begin{equation}
    ...
    P(\log Y \,|\, \log M) = \mathcal{N}(\alpha_\ym + \beta_\ym \log M, \sigma_{\rm int}^2)
\end{equation}
où $\mathcal{N}(\mu, v)$ est la distribution de probabilité d'une loi normale de moyenne $\mu$ et de variance $v$, et $\sigma_{\rm int}$ est la dispersion intrinsèque de la relation.
